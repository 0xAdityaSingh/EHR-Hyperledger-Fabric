...
app.post('/api/queryAllbyOwner', async function (req, res)  {
    ...
    // Create a new file system based wallet for managing identities.
      ...
      // Create a new gateway for connecting to our peer node.
            ...
            // queryCar transaction - requires 1 argument, ex: ('queryCar', 'CAR4')
            // queryAllCars transaction - requires no arguments, ex: ('queryAllCars')
            const result = await contract.evaluateTransaction('queryAllbyOwner');
            console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
...
// Submit the specified transaction.
        ...
        const result =await contract.submitTransaction('createCar', req.body.owner, req.body.type, req.body.data);
...
app.post('/api/register', async function (req, res)  {
    try {
        ...
        // Check to see if we've already enrolled the user.
        const userIdentity = await wallet.get(req.body.userId2);
        if (userIdentity) {
            console.log('An identity for the user "doctor1" already exists in the wallet');
            res.status(200).json({response: 'An identity for the user already exists in the wallet'});
            return;
        }

        // Check to see if we've already enrolled the admin user.
        const adminIdentity = await wallet.get(req.body.userId);
        if (!adminIdentity) {
            console.log('An identity for the admin user "admin" does not exist in the wallet');
            console.log('Run the enrollAdmin.js application before retrying');
            res.status(404).json({error: 'An identity for the admin user "admin" does not exist in the wallet'});
            return;
        }

        // build a user object for authenticating with the CA
        const provider = wallet.getProviderRegistry().getProvider(adminIdentity.type);
        const adminUser = await provider.getUserContext(adminIdentity, req.body.userId);
        ...
        const secret = await ca.register({
            ...
            role: 'client',
            attrs: [
                {"name": "hf.Registrar.Roles", "value": req.body.role}
            ],
        }, adminUser);
        ...
        await wallet.put(req.body.userId2, x509Identity);
        console.log('Successfully registered and enrolled admin user "doctor1" and imported it into the wallet');
